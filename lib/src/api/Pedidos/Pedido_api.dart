...
class PedidoApi {
  ...
  Future<dynamic> obtenerPedidosPorIdSucursal(String idSucursal) async {
    final response =
        await http.post(Uri.parse("$apiBaseURL/api/Pedido/pedidos_por_subsidiary_ws"), body: {'id_subsidiary': '$idSucursal', 'estado': '99', 'tn': prefs.token, 'app': 'true'});
...
//recorremos la lista de pedidos
    for (var i = 0; i < decodedData["result"].length; i++) {
      ...
      sucursalModel.subsidiaryCellphone2 = decodedData["result"][i]['subsidiary_cellphone_2'];
      ...
      final list = await sucursalDb.obtenerSubsidiaryPorIdSubsidiary(decodedData["result"][i]['id_subsidiary']);
      ...
      final listCompany = await companyDb.obtenerCompanyPorId(decodedData["result"][i]['id_company']);
  ...
  Future<dynamic> updateStatus(String id, String estado) async {
    try {
      final response = await http.post(Uri.parse('$apiBaseURL/api/Pedido/update_status'), body: {
        'id': '$id',
        'app': 'true',
        'tn': prefs.token,
        'status': '$estado',
